...
async function boot(){
 ...
 clients=(await api('/api/clients')).clients;renderProfiles();const job=localStorage.getItem('heatmapJob');if(job)poll(job);
...
$('clientList').onchange=e=>{if(e.target.type!=='checkbox')return;e.target.checked?selectedProfiles.add(e.target.value):selectedProfiles.delete(e.target.value);scans=[];loadedIds=[];$('step2').classList.add('hidden');if(!busy)$('step3').classList.add('hidden');renderProfiles();summary();};
